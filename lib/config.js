    ...
    tabs.open({
                ...
                onReady : function(tab) {
                    if (!/configPanel.html$/.test(tab.url)) {
                        tab.attach({
                                    contentScriptFile:[data.url('scripts/jquery-1.6.2.min.js'),data.url("scripts/help.js")]
                                });
                        return;
                    }
                    configTabWorker = tab.attach({
                                contentScriptFile:[data.url('scripts/jquery-1.6.2.min.js'),data.url('scripts/stringKeyExtractor.js'),data.url("scripts/configPanelContentScript.js")]
                            });
                    configTabWorker.port.on('save-shortcuts', function(omniShortcutData) {
                        fileIO.writeFunctionData(JSON.stringify(omniShortcutData.functionData));
