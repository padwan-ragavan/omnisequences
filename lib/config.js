...
var openConfigPanel = function() {
    tabs.open({
                ...
                onReady : function(tab) {
                    ...
                    configTabWorker.port.on('export-omnisequence', function(rules) {
                        var exportFileName = saveAsDialog.getExportFileName();
                        var omnisequences = omniShortcutDataStore.fetchOmniShortcuts();
                        var exportData = { };
                        var existingRules = omnisequences.rules;
                        var ruleMap = rules.map(function(rule) {
                            var r = { };
                            return r[rule.name] = existingRules.filter(function(er){ return er.name == rule.name;})[0];
                        });
